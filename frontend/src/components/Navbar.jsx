...
const Navbar = () => {
  ...
  const creditsRemaining = useSelector(
    (store) => store.global.creditsRemaining
  );
    ...
    try {
      ...
      if (response?.data?.success) {
        ...
        localStorage.setItem(
          "creditsRemaining",
          JSON.stringify(response.data.data.creditsRemaining)
        );
        reset();
        ...
        <div className="w-11/12 max-w-[1200px] mx-auto flex justify-between items-center py-2">
          ...
          {authenticated ? (
            <nav className="flex gap-4 items-center">
              <Link
                to="/buy-credit"
                className="flex items-center gap-2 text-white bg-btn-primary hover:scale-105 duration-300 px-4 py-2 rounded-full"
              >
                <Star className="hidden md:block" size={20} />
                ...
                <div className="hidden bg-white group-hover:block absolute top-[100%] left-0 shadow-[0_8px_30px_rgb(0,0,0,0.12)]">
                  <ul>
                    <button
                      ...
                      onClick={logout}
                      className="px-3 py-1 cursor-pointer hover:bg-[rgb(230,230,230)] "
                    >
                      Logout
                    </button>
                  </ul>
              ...
              <div
                className={`${
                  isMobileModalVisible ? "block" : "hidden"
                } fixed h-screen w-full backdrop-blur-md pt-10 left-0 top-0`}
              >
                ...
                <p
                  ...
                  className="text-text-primary text-3xl font-medium text-center pt-20"
                >
                  Logout
                </p>
              </div>
            ...
            <nav className="flex gap-4 items-center">
              <Link to="/buy-credit" className="text-white">
                Pricing
              </Link>
              <button
                onClick={() => {
                ...
                className="bg-btn-primary text-white px-6 py-2 cursor-pointer rounded-full"
              >
                Login
              </button>
            </nav>
      ...
      <AnimatePresence>
        {isModalVisible ? (
          <motion.div
            ...
            className={`fixed z-50 top-0 left-0 bg-black/30 h-screen w-full backdrop-blur-md flex justify-center items-center`}
          >
            <div
              onClick={(e) => {
                e.stopPropagation();
              }}
              className="bg-white rounded-lg pt-4 pb-10 px-4"
            >
              <div className="flex justify-end text-text-primary cursor-pointer">
                <X
                  onClick={() => {
                    dispatch(setIsModalVisible(false));
                  }}
              ...
              >
                ...
                >
                  <Link to="/forgot-password" className="text-[rgb(0,122,255)]">
                    Forgot password?
                  </Link>
                </p>
                <button
              ...
              >
                ...
                <Link to="/signup" className="text-[rgb(0,122,255)] ml-1">
                  Sign up
                </Link>
              </p>
            </div>
