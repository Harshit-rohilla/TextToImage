...
import { Eye, EyeSlash, ArrowLeft, ArrowClockwise } from "@phosphor-icons/react";
import { useState } from "react";
import { Link, useNavigate} from "react-router-dom";
import { useDispatch, useSelector } from "react-redux";
import { setIsModalVisible, setSignupFormData } from "../redux/slices/globalSlice";
...
//   *register user in db for first time
  const registerUser=async()=>{
    ...
    try {
        const response=await axios.post(`${import.meta.env.VITE_BASE_URL}/user/signup`,{...signupFormData,otp})
...
//   *sending or resending otp to user's email 
  const sendOtp = async (data) => {
    ...
    try {
        const response=await axios.post(`${import.meta.env.VITE_BASE_URL}/user/otp`,{email:data.email})
        if(response.data?.success){
      ...
      <div className="flex-1 flex flex-col justify-center items-center">
        {isOtpSend ? (
          <div className="bg-white rounded-lg py-6 px-4 shadow-[0_8px_30px_rgb(0,0,0,0.12)]">
            <h1 className="text-text-primary text-3xl font-medium text-center mb-4">Verify OTP</h1>
            <OtpInput
                value={otp}
                onChange={setOtp}
                numInputs={6}
            ...
            />
            <button onClick={registerUser} className="bg-btn-primary w-full py-2 mt-4 rounded-md text-white text-center cursor-pointer" disabled={loading}>Submit</button>
            <div className="flex justify-between mt-2 text-text-primary">
            <button onClick={()=>{setIsOtpSend(false)}} className="cursor-pointer"><ArrowLeft/></button>
            <button disabled={loading} onClick={()=>{sendOtp(signupFormData)}} className="flex gap-1 cursor-pointer items-center">Resend <ArrowClockwise/></button>
            </div>
          </div>
        ) : (
          <div className="bg-white rounded-lg pt-4 pb-10 px-4">
            ...
            <p className="flex justify-center mt-5">
              ...
              <Link onClick={()=>{dispatch(setIsModalVisible(true))}} to="/" className="text-[rgb(0,122,255)] ml-1">
